...
public class RedBlackTree {
	...
	public RedBlackTree(String s){
		NIL.setColor(BLACK);
		root = new RedBlackNode(s, RED, NIL, NIL, NIL);
		size = 1;
		recentComp = 0;
	}
	
	/**
	 * find a value close to v in the tree, if v is found in the tree it returns v<br>
	 * pre-condition: none<br>
	 * post-condition: returns a value close to v in the tree, if v is found in the tree it returns v<br>
	 ...
	 * @return returns a value close to v in the tree, if v is found in the tree it returns v
	 */
	public String closeBy(String v){
		RedBlackNode t = root;
		while(t != NIL){
			if(t.getData().compareTo(v) == 0)	return v;
			else if (t.getData().compareTo(v) > 0){
			//else if (t.getData().compareTo(v) > 0 && !t.getData().contains(v)){
				if(t.getLc() != NIL)	t = t.getLc();
				else	return t.getData();
			}
			else{
				if(t.getRc() != NIL) t = t.getRc();	
				else	return t.getData();
			}
		}
		return "";
